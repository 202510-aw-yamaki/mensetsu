...
- PC幅の沈み込みは、`slide-body` の 750px 固定だけでは解消しなかった
- 原因は `usage-device-stack` と `usage-device-visual` が内容量基準のままで、750px 内に収まる前提になっていなかったこと
- PC幅では左カラムを `height: 100%` の flex に寄せ、右カラム画像も `contain` 前提で枠内に収める方針に変更した
